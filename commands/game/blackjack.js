...
module.exports = {
  ...
  code: async (ctx) => {
    ...
    try {
      ...
      if (chips < bet) {
        sessions.delete(chatId);
        return ctx.reply(quote("❎ Anda tidak memiliki cukup chip untuk bertaruh!"));
      }
      // deduct bet up front
      await db.add(`user.${playerId}.chips`, -bet);
      ...
      collector.on('collect', async m => {
        ...
        if (cmd === 'hit') {
          ...
          if (dealerScoreFinal > 21 || playerScoreFinal > dealerScoreFinal) {
            await db.add(`user.${playerId}.chips`, bet * 2);
          ...
          const summary =
            ...
            `\n${resultMsg} ${resultMsg.includes('menang')?`Anda mendapat ${bet} chip.`:resultMsg.includes('Draw')?`Taruhan Anda dikembalikan.`:`Anda kalah ${bet} chip.`}`;
          sessions.delete(chatId);
