...
module.exports = {
  ...
  code: async (ctx) => {
    ...
    try {
      ...
      const balance = userDb.chips || 0;

      if (balance < bet) {
        sessions.delete(chatId);
        return ctx.reply(quote("❎ Anda tidak memiliki cukup chips!"));
      }

      // deduct initial bet
      await db.subtract(`user.${playerId}.chips`, bet);
      ...
      if (!win) {
        sessions.delete(chatId);
        return ctx.reply(quote(
          `💔 Anda kalah ${monospace(bet + " chips")} pada flip pertama!`
        ));
      }

      // on win, prompt for take or flip
      await ctx.reply(
        `${quote(`🎉 Anda menang! Kemenangan: ${monospace(current + " chips")}`)}
` +
      ...
      collector.on('collect', async m => {
        ...
        if (cmd === 'ambil') {
          // cash out
          await db.add(`user.${playerId}.chips`, current);
          sessions.delete(chatId);
          await ctx.sendMessage(chatId, { text: quote(
            `💰 Anda mengambil ${monospace(current + " chips")}! Selamat!`
          )}, { quoted: m });
          collector.stop();
        } else if (cmd === 'double') {
          // attempt next flip
          ...
          if (!winNext) {
            sessions.delete(chatId);
            await ctx.sendMessage(chatId, { text: quote(
              `💥 Anda kalah segalanya! Kemenangan terakhir adalah ${monospace(current + " chips")}. Lebih baik beruntung lain waktu.`
            )}, { quoted: m });
            collector.stop();
          } else {
            // double current
            current *= 2;
            await ctx.sendMessage(chatId, { text: quote(
              `🎉 Bagus! Kemenangan berikutnya: ${monospace(current + " chips")}`
            )}, { quoted: m });
